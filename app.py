            ...
            if st.button("🤖 Generate AI Report", type="primary"):
                try:
                    with st.spinner("AI is generating your comprehensive report... This may take a minute."):
                        ...
                        st.session_state.show_current_report = True
                        st.session_state.show_full_report = False
                        st.session_state.show_metrics = include_charts
                except Exception as e:
                    handle_error(e)
                    st.session_state.show_current_report = False
                    st.session_state.show_full_report = False
                    st.session_state.show_metrics = False
        
        # Initialize show states if not present
        if 'show_current_report' not in st.session_state:
            st.session_state.show_current_report = False
        if 'show_full_report' not in st.session_state:
            st.session_state.show_full_report = False

        if 'generated_report' in st.session_state and st.session_state.get('show_current_report', False):
            st.markdown("---")
